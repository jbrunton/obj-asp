...
%token <Support.Error.info> METHOD
%token <Support.Error.info> MMETHOD
%token <Support.Error.info> FST
%token <Support.Error.info> SND
%token <Support.Error.info> ASP
%token <Support.Error.info> AASP
%token <Support.Error.info> LAB
%token <Support.Error.info> LLAB
%token <Support.Error.info> NEWLAB
%token <Support.Error.info> ASPECT
%token <Support.Error.info> ADVICE
%token <Support.Error.info> JOIN
%token <Support.Error.info> TERM
%token <Support.Error.info> TYPE
...
%token <Support.Error.info> UPDARROW
...
Type :
  ...
  | MMETHOD AType
      { fun ctx -> TyMethod($2 ctx) }
  | AASP AType
      { fun ctx -> TyAsp($2 ctx) }
  | LLAB AType
      { fun ctx -> TyLab($2 ctx) }
  | JOIN LSQUARE LCID RSQUARE Type
      { fun ctx -> StTyJoin($3.v, $5 ctx) }
...
AType :
  ...
  | NAT
      { fun ctx -> TyNat }
  | LSQUARE MemberTypes RSQUARE
      { fun ctx ->
          TyObject($2 ctx) }
  | LPAREN Type STAR Type RPAREN
      { fun ctx -> TyPair($2 ctx,$4 ctx) }
...
Term :
  ...
  | METHOD LPAREN LCID RPAREN COLON Type DOT Term 
      { fun ctx ->
          let ctx1 = addname ctx $3.v in
          TmMethod($1, $3.v, $6 ctx, $8 ctx1) }
  ...
  | Term BARGT Term
      { fun ctx -> TmInstall($2, $1 ctx, $3 ctx) }
  | ASPECT LCID COLON TERM IN Term
      { fun ctx -> StAsp($1, $2.v, $6 ctx) }
  | ASPECT LCID COLON TYPE IN Term
      { fun ctx -> StTyAsp($1, $2.v, $6 ctx) }
  | ADVICE LCID LEFTARROW LPAREN Members RPAREN IN Term
      { fun ctx ->
          StObjAdv($1, $2.v, $5 ctx, $8 ctx) }
  | ADVICE LCID LEFTARROW LPAREN MemberTypes RPAREN IN Term
      { fun ctx ->
          StObjTyAdv($1, $2.v, $5 ctx, $8 ctx) }
  | JOIN LSQUARE LCID RSQUARE Term
      { fun ctx -> StJoin($1, $3.v, $5 ctx) }
...
AppTerm :
  ...
  | PathTerm DOT LCID LT METHOD LPAREN LCID RPAREN COLON Type DOT Term
      { fun ctx -> 
          let ctx1 = addname ctx $7.v in
          TmObjUpdate($2, $1 ctx, $3.v, $7.v, $10 ctx, $12 ctx1) }
  | FST PathTerm
      { fun ctx -> TmFst($1, $2 ctx) }
  | SND PathTerm
      { fun ctx -> TmSnd($1, $2 ctx) }
  | NEWLAB AType
      { fun ctx -> TmNewLab($1, $2 ctx) }
  | PathTerm LT Term GT
      { fun ctx -> TmJoin($2, $1 ctx, $3 ctx) }
...
NEMemberTypes :
    MemberType
      { fun ctx -> [$1 ctx] }
  | MemberType COMMA NEMemberTypes
      { fun ctx -> ($1 ctx) :: ($3 ctx) }
  ...
  | INTV
      { fun ctx ->
          let rec f n = match n with
            ...
            | n -> TmSucc($1.i, f (n-1))
          in f $1.v }
  | LSQUARE Members RSQUARE
      { fun ctx ->
          TmObject($1, $2 ctx) }
  | LPAREN Term COMMA Term RPAREN
      { fun ctx -> TmPair($1,$2 ctx,$4 ctx) }
  | LCURLY ATerm DOT LCID ARROW Term RCURLY
      { fun ctx -> TmAsp($1, $2 ctx, $4.v, $6 (addname ctx ($4.v))) }
